 ...
 */
VisionTree::VisionTree(std::string name, float wanted_frequency) {
  must_stop_ = true;
  thread_ = nullptr;
  queue_ = nullptr;
 ...
 * @return Validation
 */
bool VisionTree::AddNode(std::string type,
                         std::string id,
 ...
 * @param outputData
 */
void VisionTree::CallbackFunction(VisionNode* visionNode, Data outputData) {
  // Add visionNode and data in Queue
  queue_->Enqueue(visionNode, outputData);
}
...
void VisionTree::Start() {
  ...
  thread_ = new boost::thread(boost::bind(&VisionTree::Thread, this));
}
 ...
 * @param wanted_frequency
 */
void VisionTree::set_wanted_frequency(float wanted_frequency) {
  boost::unique_lock<boost::mutex> lock(wanted_frequency_mutex_);
...
float VisionTree::current_frequency() {
  boost::unique_lock<boost::mutex> lock(current_frequency_mutex_);
